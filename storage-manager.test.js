...
if (typeof window === 'undefined') {
    // Create a mock body for document operations
    const mockBody = {
        appendChild: (element) => {
            mockBody.children = mockBody.children || [];
            mockBody.children.push(element);
        },
        removeChild: (element) => {
            if (mockBody.children) {
                const index = mockBody.children.indexOf(element);
                if (index > -1) {
